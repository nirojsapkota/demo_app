class AffiliationsController < ApplicationController
  ...
  def index
    if params[:query] && !params[:query].blank?
      @affiliations = Affiliation.where(name: params[:query]).paginate(page: params[:page], per_page: 10)
    else
      @affiliations = Affiliation.paginate(page: params[:page], per_page: 10)
    end
  end
  ...
  private
    ...
    def affiliation_params
      params.require(:affiliation).permit(:name, :person_id)
    end
end
